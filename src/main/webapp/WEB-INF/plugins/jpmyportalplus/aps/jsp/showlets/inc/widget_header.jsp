...
<%-- moveActionURL: il path all'action che si occupa di mostrare o nascondere il form per lo spostamento --%>
	...
	<c:set var="configureActionUrl"><wp:url paramRepeat="false" />?configureFrame=<c:out value="${editFrameValue}" escapeXml="false" />&currentPageCode=<wp:currentPage param="code" escapeXml="false" /></c:set>
	...
	<div class="boxspacer"> <%-- classe spaziatrice: serve per dare eventuale del padding (non esiste in nessun css, se non serve rimuovere) --%>
		<div class="dragdrop_draggable_area"> <%-- class="dragdrop_draggable_area": classe css referenziata in jpmyportalplus.js, serve per dichiarare qual'è l'elemento "maniglia" per il trascinamento. --%>
				<div class="shortcut">
					...
					<a href="<c:out value="${opencloseActionURL}" />" class="button openClose" title="open close"><img src="<wp:resourceURL />plugins/jpmyportalplus/static/img/closed-<c:out value="${isClosed}" />.png" alt="open/close" /></a>&#32;<%-- class="openClose": referenziato in jpmyportalplus.js, apre e chiude il contenuto di "editcontentBox" e "content" --%>
					...
					<a href="#<c:out value="${showletId}" />-configure" class="button configure" title="configure"><img src="<wp:resourceURL />plugins/jpmyportalplus/static/img/configure.png" alt="configure" /></a>&#32;<%-- class="openClose": referenziato in jpmyportalplus.js, apre e chiude il contenuto di "editcontentBox" e "content" --%>
				</div>
				<c:choose>
		...
		<div id="<c:out value="${showletId}" />-configure" class="configure-box">
			<div class="spacer">
