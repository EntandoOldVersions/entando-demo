						...
						<li <c:if test="${status.last}">class="last_comment"</c:if>> 
							...
							<div class="comment_info">
								...
								<p> 
									...
									<c:set var="ddd" value="${comment.creationDate}" scope="request" />
									<s:date name="%{#request.ddd}" nice="true" format="dd/MM/yyyy HH:mm:ss" />
									<c:set var="ddd" value="${null}" />
							...
							<c:if test="${isUsedCommentWithRating}" >
								<div class="comment_rating">
									...
									<s:if test="#commentRating != null">
										<h4 class="comment_current_average_vote"><wp:i18n key="jpcontentfeedback_COMMENT_RATING" />&#32;<fmt:formatNumber value="${commentRating.average}" pattern="#0.00" /></h4>
										<p>
											<c:out value="${commentRating.voters}" />&#32;<wp:i18n key="jpcontentfeedback_VOTERS_NUM" />
										</p>
									</s:if>
									<s:else>
