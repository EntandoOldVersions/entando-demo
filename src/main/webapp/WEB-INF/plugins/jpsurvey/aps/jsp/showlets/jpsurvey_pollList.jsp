...
<wp:info key="defaultLang" var="defaultLang" />
	...
	<jpsu:surveyList ctxName="pollList" category="poll" expired="false" />
					...
					<h2>
						<c:choose>
							<c:when test="${not empty currentSurvey.titles[currentLang]}">
								<a href="<wp:url page="poll_detail"><wp:parameter name="surveyId"><c:out value="${currentSurvey.id}"/></wp:parameter></wp:url>" title="<wp:i18n key="JPSURVEY_GO_TO_POLL" />:&#32;<c:out value="${currentSurvey.titles[currentLang]}"/>"><c:out value="${img}" escapeXml="false" /><c:out value="${currentSurvey.titles[currentLang]}"/></a>		
							</c:when>
							<c:otherwise>
								<a href="<wp:url page="poll_detail"><wp:parameter name="surveyId"><c:out value="${currentSurvey.id}"/></wp:parameter></wp:url>" title="<wp:i18n key="JPSURVEY_GO_TO_POLL" />:&#32;<c:out value="${currentSurvey.titles[defaultLang]}"/>"><c:out value="${img}" escapeXml="false" /><c:out value="${currentSurvey.titles[defaultLang]}"/></a>
							</c:otherwise>
						</c:choose>
					</h2>
		...
		<p class="archive_link"><wp:i18n key="JPSURVEY_POLLS_LIST_INTRO" />&#32;<a href="<wp:url page="polls_archive"></wp:url>" title="<wp:i18n key="JPSURVEY_GO_TO_POLL_ARCHIVE" />"><wp:i18n key="JPSURVEY_POLL_ARCHIVE" /></a>.</p>
