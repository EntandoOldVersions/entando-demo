		...
		<h2>
			<c:choose>
				<c:when test="${not empty currentSurvey.titles[currentLang]}">
					<a href="<wp:url page="survey_detail"><wp:parameter name="surveyId"><c:out value="${currentSurvey.id}"/></wp:parameter></wp:url>" title="<wp:i18n key="JPSURVEY_GO_TO_QUESTIONNAIRE" />:&#32;<c:out value="${currentSurvey.titles[currentLang]}"/>">
					<c:out value="${img}" escapeXml="false" />
					<c:out value="${currentSurvey.titles[currentLang]}"/></a>		
				</c:when>
				<c:otherwise>
					<a href="<wp:url page="survey_detail"><wp:parameter name="surveyId"><c:out value="${currentSurvey.id}"/></wp:parameter></wp:url>" title="<wp:i18n key="JPSURVEY_GO_TO_QUESTIONNAIRE" />:&#32;<c:out value="${currentSurvey.titles[defaultLang]}"/>">
					<c:out value="${img}" escapeXml="false" />
					<c:out value="${currentSurvey.titles[defaultLang]}"/></a>
				</c:otherwise>
			</c:choose>
		</h2>
		<p class="note"><span class="small">(<c:choose><c:when test="${voted}"><wp:i18n key="JPSURVEY_YOU_HAVE_VOTED" /></c:when><c:otherwise><wp:i18n key="JPSURVEY_YOU_HAVE_NOT_VOTED" /></c:otherwise></c:choose>)</span>			
		<p class="descr">
		<c:choose>
