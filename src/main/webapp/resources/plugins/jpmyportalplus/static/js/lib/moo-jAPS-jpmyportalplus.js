...
script: moo-jAPS-jpmyportalplus.js
...
var MOO_JAPS_JPMYPORTALPLUS = new Class({
	...
	applyLoading: function(item) {
		if (item!==null&&item.getElement(this.options.selettoreApriChiudi)!=null){
			this._loadingEl.clone().inject(item.getElement(this.options.selettoreApriChiudi).getParent());
		}
	},
		...
		for (var i = 0; i < boxes.length; i++) {
			...
			if ($defined(openEl) && $defined(boxApriChiudi)) {
				var fxopenEl = new Fx.Reveal(boxApriChiudi,{
					...
					"mode": "vertical",
					onShow: function(box) {
						this[0].fireEvent("boxShow", this[1]);
					}.bind([this, item]),
					onHide: function(box) {
						this[0].fireEvent("boxHide", this[1]);
					}.bind([this, item])
				});
